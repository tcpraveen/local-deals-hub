...
if st.session_state.current_view == "customer":
    ...
    for idx, cat in enumerate(categories):
        if cat_cols[idx].button(cat, use_container_width=True):
            selected_cat = cat
    ...
    for deal in deals:
        ...
        <div style='font-family: sans-serif; width:160px;'>
            <b>{deal.get('shop_name', 'Local Shop')}</b><br>
        ...
        folium.Marker(
            ...
            tooltip=deal.get('shop_name', 'Local Shop'),
    ...
    for idx, deal in enumerate(deals):
        with deal_cols[idx % 3]:
            ...
            msg = f"Hi {deal.get('shop_name')}, I saw your deal '{deal.get('title')}' on Neighborhood Deals Hub!"
            wa_url = f"https://wa.me/{wa_num}?text={msg.replace(' ', '%20')}"
            
            st.markdown(f"""
                <a href="{wa_url}" target="_blank" style="text-decoration:none;">
                    <div style="background-color:#25D366; color:white; text-align:center; padding:8px; border-radius:6px; font-weight:bold;">
                        💬 Chat on WhatsApp
                    </div>
                </a>
            """, unsafe_allow_html=True)
